...
public class Listener implements org.bukkit.event.Listener {
	...
	private static HashMap<Integer, Item> magicItems = new HashMap<Integer, Item>();
	
	private TestMain plugin;
	
	public Listener(TestMain plugin) {
		this.plugin = plugin;
	}
	...
	public void EntityDeath(EntityDeathEvent event)
	{
		if (event.getEntityType() == EntityType.ZOMBIE)
		{
			...
			NMSOcelot ocelot = new NMSOcelot(plugin, world);
		...
		else if (event.getEntityType() == EntityType.OCELOT)
		{
			...
			this.plugin.getDatabase().saveData(
		...
		ChannelDuplexHandler duplex = new ChannelDuplexHandler() {
			...
			public void write(ChannelHandlerContext context, Object packet, ChannelPromise promise) throws Exception
			{
				...
				{	
					...
					int entityId = (int) plugin.getReflectionHelper().GetPrivateField("a", PacketPlayOutEntityMetadata.class, (PacketPlayOutEntityMetadata) packet);
